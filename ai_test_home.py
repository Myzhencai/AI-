...
def generatetestscripts(input_text):
    ...
    prompt = f"""
你是一个熟练的 Linux Shell 脚本工程师，请根据以下自然语言描述生成一个完整的 Shell 脚本：

执行规则：
    1、优先从函数库中选择测试函数：如果函数库中存在可直接用于测试的函数，优先调用这些函数；确保测试覆盖核心功能、边界条件和异常情况。
    2、若库中无合适函数，则自动生成测试用例：生成的测试用例应独立、可执行。
    3、禁止使用 adb shell 关键字，确保测试可移植且安全。
    4、输出格式（根据需求选择）：脚本以 #!/bin/sh 开头；没有用到的函数不用显示出来，简洁易懂。
    5、函数库如下：
{content}
